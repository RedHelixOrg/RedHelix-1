...
public class DataProvider {
  ...
  public void delete(final EdmEntitySet edmEntitySet, final Entity entity)
      throws DataProviderException {
  ...
  private boolean isFree(final Object value, final String keyPropertyName,
  ...
  private void applyNavigationBinding(final String rawBaseUri, final EdmEntitySet edmEntitySet,
    ...
    for (final Link link : navigationBindings) {
      final EdmNavigationProperty edmNavProperty =
          edmEntitySet.getEntityType().getNavigationProperty(link.getTitle());
  ...
  private void handleDeepInsert(final String rawBaseUri, final EdmEntitySet edmEntitySet,
      final Entity entity, final Entity changedEntity) throws DataProviderException {
    final EdmEntityType entityType = edmEntitySet.getEntityType();
    ...
    for (final String navPropertyName : entityType.getNavigationPropertyNames()) {
      ...
      if (navigationLink != null) {
        // Deep inserts are not allowed in update operations, so we can be sure, that we do not
        // override
        // a navigation link!
        final EdmNavigationProperty navigationProperty =
            entityType.getNavigationProperty(navPropertyName);
        final EdmEntitySet target =
            (EdmEntitySet) edmEntitySet.getRelatedBindingTarget(navPropertyName);
  ...
  private List<Entity> createInlineEntities(final String rawBaseUri,
      final EdmEntitySet targetEntitySet, final EntityCollection changedEntitySet)
          throws DataProviderException {
  ...
  public EntityCollection readFunctionEntitySet(final EdmFunction function,
      final List<UriParameter> parameters) throws DataProviderException {
  ...
  public Property readFunctionPrimitiveComplex(final EdmFunction function,
      final List<UriParameter> parameters) throws DataProviderException {
    return FunctionData.primitiveComplexFunction(function.getName(), parameters, data, odata);
  }

  public Property processActionPrimitive(final String name,
  ...
  public Property processActionComplex(final String name,
      final Map<String, Parameter> actionParameters) throws DataProviderException {
    return ActionData.complexAction(name, actionParameters);
  }

  public Property processActionComplexCollection(final String name,
      final Map<String, Parameter> actionParameters) throws DataProviderException {
    return ActionData.complexCollectionAction(name, actionParameters);
  }

  public Property processActionPrimitiveCollection(final String name,
      final Map<String, Parameter> actionParameters) throws DataProviderException {
    return ActionData.primitiveCollectionAction(name, actionParameters, odata);
  }

  public EntityActionResult processActionEntity(final String name,
      final Map<String, Parameter> actionParameters) throws DataProviderException {
  ...
  public void createReference(final Entity entity, final EdmNavigationProperty navigationProperty,
      final URI entityId, final String rawServiceRoot) throws DataProviderException {
    setLink(navigationProperty, entity,
        getEntityByReference(entityId.toASCIIString(), rawServiceRoot));
  }
