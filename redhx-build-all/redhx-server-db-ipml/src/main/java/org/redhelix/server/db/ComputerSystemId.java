 ...
 *
 */
final class ComputerSystemId
        extends AbstractPersistantId
        implements RedHxDbComputerId
{

    private static final String ID_PREFIX = "Com";

    ComputerSystemId(int seqId)
    {
        super(seqId,
              ID_PREFIX);
    }
    ...
    @Override
    public int compareTo(RedHxDbComputerId other)
    {
        int retVal = this.getSeqId() - super.getSeqId();

        if (retVal != 0)
        {
            retVal = (this.getSeqId() < super.getSeqId())
                    ? -1
                    : 1;
        }
