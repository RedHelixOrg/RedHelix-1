...
public abstract class Commons {
  ...
  static {
    ...
    MEDIA_CONTENT.put("CustomerInfo", new ImmutablePair<String, EdmPrimitiveTypeKind>(
        "CustomerInfoId", EdmPrimitiveTypeKind.Int32));
  ...
  public static Metadata getMetadata(final ODataServiceVersion version) {
    if (!METADATA.containsKey(version)) {
      final InputStream is =
          Commons.class.getResourceAsStream("/" + version.name() + "/metadata.xml");
  ...
  public static String getLinksURI(final String entitySetName, final String entityId,
  ...
  public static String getLinksPath(final String basePath, final String linkName,
      final Accept accept) throws IOException {
    try {
      return FSManager.instance(ODataServiceVersion.V40).getAbsolutePath(
          basePath + Constants.get(ConstantKey.LINKS_FILE_PATH) + File.separatorChar + linkName,
          accept);
    } catch (Exception e) {
  ...
  public static InputStream getLinksAsATOM(final Map.Entry<String, Collection<String>> link)
    ...
    builder.append("<links xmlns=\"").append(Constants.get(ConstantKey.DATASERVICES_NS))
        .append("\">");
  ...
  public static InputStream getLinksAsJSON(final String entitySetName,
    ...
    links.put(Constants.get(ConstantKey.JSON_ODATAMETADATA_NAME),
        Constants.get(ConstantKey.ODATA_METADATA_PREFIX) + entitySetName + "/$links/"
            + link.getKey());
      ...
      case JSON:
        ...
        while (fields.hasNext()) {
          ...
          if (field.getKey().endsWith(Constants.get(ConstantKey.JSON_MEDIA_SUFFIX))
            ...
            for (final Iterator<JsonNode> subItor = field.getValue().elements(); subItor
                .hasNext();) {
  ...
  public static String getETag(final String basePath) throws Exception {
    try {
      final InputStream is =
          FSManager.instance(ODataServiceVersion.V40).readFile(basePath + "etag", Accept.TEXT);
      if (is.available() <= 0) {
        return null;
      } else {
