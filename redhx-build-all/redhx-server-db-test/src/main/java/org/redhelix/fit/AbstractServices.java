...
import org.redhelix.fit.metadata.EntityType;
import org.redhelix.fit.metadata.Metadata;
import org.redhelix.fit.metadata.NavigationProperty;
import org.redhelix.fit.methods.MERGE;
import org.redhelix.fit.methods.PATCH;
import org.redhelix.fit.serializer.FITAtomDeserializer;
import org.redhelix.fit.utils.AbstractUtilities;
import org.redhelix.fit.utils.Accept;
import org.redhelix.fit.utils.Commons;
import org.redhelix.fit.utils.ConstantKey;
import org.redhelix.fit.utils.Constants;
import org.redhelix.fit.utils.FSManager;
import org.redhelix.fit.utils.JSONUtilities;
import org.redhelix.fit.utils.LinkInfo;
import org.redhelix.fit.utils.XMLUtilities;
...
public abstract class AbstractServices {
  ...
  private static final Pattern BATCH_REQUEST_REF_PATTERN =
      Pattern.compile("(.*) ([$]\\d+)(.*) HTTP/.*");
  ...
  public AbstractServices(final ODataServiceVersion version, final Metadata metadata)
      throws IOException {
  ...
  protected Response getMetadata(final String filename) {
    try {
      return xml.createResponse(null, FSManager.instance(version).readRes(filename, Accept.XML),
          null, Accept.XML);
    } catch (Exception e) {
  ...
  public Response patchDriver() {
    return xml.createFaultResponse(Accept.JSON_FULLMETA.toString(),
        new Exception("Non nullable properties"));
  }
  ...
  public Response getStoredPI(@Context final UriInfo uriInfo) {
    ...
    try {
      ...
      return xml.createResponse(new ByteArrayInputStream(content.toByteArray()), null,
  ...
  protected abstract InputStream exploreMultipart(final List<Attachment> attachments,
      final String boundary, final boolean continueOnError) throws IOException;

  protected void addItemIntro(final ByteArrayOutputStream bos) throws IOException {
    addItemIntro(bos, null);
  }

  protected void addItemIntro(final ByteArrayOutputStream bos, final String contentId)
      throws IOException {
  ...
  protected void addChangesetItemIntro(final ByteArrayOutputStream bos, final String contentId,
  ...
  protected void addSingleBatchResponse(final Response response, final ByteArrayOutputStream bos)
      throws IOException {
    addSingleBatchResponse(response, null, bos);
  }

  protected void addSingleBatchResponse(final Response response, final String contentId,
  ...
  protected void addErrorBatchResponse(final Exception e, final String contentId,
      final ByteArrayOutputStream bos) throws IOException {
  ...
  public Response mergeEntity(@Context final UriInfo uriInfo,
      @HeaderParam("Accept") @DefaultValue(StringUtils.EMPTY) final String accept,
      @HeaderParam("Content-Type") @DefaultValue(StringUtils.EMPTY) final String contentType,
      @HeaderParam("Prefer") @DefaultValue(StringUtils.EMPTY) final String prefer,
      @HeaderParam("If-Match") @DefaultValue(StringUtils.EMPTY) final String ifMatch,
      @PathParam("entitySetName") final String entitySetName,
      @PathParam("entityId") final String entityId, final String changes) {

    return patchEntity(uriInfo, accept, contentType, prefer, ifMatch, entitySetName, entityId,
        changes);
  }
  ...
  public Response patchEntity(@Context final UriInfo uriInfo,
    ...
    try {
      ...
      final Map.Entry<String, InputStream> entityInfo =
          xml.readEntity(entitySetName, entityId, Accept.ATOM);
      ...
      if (contentTypeValue == Accept.XML || contentTypeValue == Accept.TEXT) {
        ...
        final ResWrap<Entity> jcont =
            jsonDeserializer.toEntity(IOUtils.toInputStream(changes, Constants.ENCODING));
      ...
      FSManager.instance(version).putInMemory(cres,
          path + File.separatorChar + Constants.get(ConstantKey.ENTITY));
      ...
      if ("return-content".equalsIgnoreCase(prefer)) {
        ...
        res.close();
        response = xml.createResponse(uriInfo.getRequestUri().toASCIIString(), null, null,
  ...
  public Response replaceEntity(@Context final UriInfo uriInfo,
      ...
      @HeaderParam("Prefer") @DefaultValue(StringUtils.EMPTY) final String prefer,
      @PathParam("entitySetName") final String entitySetName,
      @PathParam("entityId") final String entityId, final String entity) {
  ...
  public Response postNewEntity(@Context final UriInfo uriInfo,
    ...
    try {
      ...
      final org.redhelix.fit.metadata.EntitySet entitySet = metadata.getEntitySet(entitySetName);
        ...
        if (id != null) {
          ...
          prop.setValue(ValueType.PRIMITIVE,
              id.getValue() == EdmPrimitiveTypeKind.Int32 ? Integer.parseInt(entityKey)
                  : id.getValue() == EdmPrimitiveTypeKind.Guid ? UUID.fromString(entityKey)
                      : entityKey);
      ...
      if ("return-no-content".equalsIgnoreCase(prefer)) {
        ...
        response = xml.createResponse(location, xml.writeEntity(acceptType, result), null,
            acceptType, Response.Status.CREATED);
      }
  ...
  public Response actionSack(
      @HeaderParam("Accept") @DefaultValue(StringUtils.EMPTY) final String accept,
      @PathParam("entityId") final String entityId, @PathParam("type") final String type,
    ...
    try {
      final Map.Entry<String, InputStream> entityInfo =
          xml.readEntity("Person", entityId, Accept.ATOM);
      ...
      return utils.getValue().createResponse(null, null, null, utils.getKey(),
  ...
  public Response actionIncreaseSalaries(
    ...
    try {
      ...
      final Pattern salary = Pattern.compile(acceptType == Accept.ATOM
          ? "\\<d:Salary m:type=\"Edm.Int32\"\\>(-?\\d+)\\</d:Salary\\>" : "\"Salary\":(-?\\d+),");
  ...
  public Response actionChangeProductDimensions(
    ...
    try {
      final Map.Entry<String, InputStream> entityInfo =
          xml.readEntity("Product", entityId, Accept.ATOM);

      final InputStream entity = entityInfo.getValue();
      final ResWrap<Entity> container = atomDeserializer.toEntity(entity);

      final Entity param =
          xml.readEntity(utils.getKey(), IOUtils.toInputStream(argument, Constants.ENCODING));

      final Property property = param.getProperty("dimensions");
      container.getPayload().getProperty("Dimensions").setValue(property.getValueType(),
          property.getValue());
  ...
  public Response actionResetComputerDetailsSpecifications(
    ...
    try {
      final Map.Entry<String, InputStream> entityInfo =
          xml.readEntity("ComputerDetail", entityId, Accept.ATOM);
      ...
      try {
        ...
        if (StringUtils.isNotBlank(filter)) {
          ...
          builder.append(metadata.getEntitySet(name).isSingleton()
              ? Constants.get(ConstantKey.ENTITY) : Constants.get(ConstantKey.FEED));
        }
  ...
  public Response getEntitySet(@Context final UriInfo uriInfo,
    ...
    return getEntitySet(uriInfo, accept, name, top, skip, format, count, filter, orderby, skiptoken,
        null);
  }
  ...
  public Response getPerson(@Context final UriInfo uriInfo,
    ...
    final Response internal = getEntityInternal(uriInfo.getRequestUri().toASCIIString(), accept,
        "Person", entityId, format, null, null);
    if (internal.getStatus() == 200) {
      ...
      try {
        if (utils.getKey() == Accept.JSON_FULLMETA || utils.getKey() == Accept.ATOM) {
          entity = utils.getValue()
              .addOperation(entity, "Sack", "#DefaultContainer.Sack", uriInfo.getAbsolutePath()
                  .toASCIIString()
  ...
  public Response getProduct(@Context final UriInfo uriInfo,
    ...
    final Response internal = getEntityInternal(uriInfo.getRequestUri().toASCIIString(), accept,
        "Product", entityId, format, null, null);
    if (internal.getStatus() == 200) {
      InputStream entity = (InputStream) internal.getEntity();
      try {
        if (utils.getKey() == Accept.JSON_FULLMETA || utils.getKey() == Accept.ATOM) {
  ...
  public Response getEntity(@Context final UriInfo uriInfo,
    ...
    return getEntityInternal(uriInfo.getRequestUri().toASCIIString(), accept, entitySetName,
        entityId, format, expand, select);
  }

  protected Response getEntityInternal(final String location, final String accept,
      final String entitySetName, final String entityId, final String format, final String expand,
      final String select) {
    ...
    try {
      ...
      if ((this instanceof KeyAsSegment)) {
        ...
        editLink.setTitle(entitySetName);
        editLink.setHref(
            Constants.get(ConstantKey.DEFAULT_SERVICE_URL) + entitySetName + "/" + entityId);
  ...
  public Response getMediaEntity(@Context final UriInfo uriInfo,
    ...
    try {
      ...
      final Map.Entry<String, InputStream> entityInfo =
          utils.readMediaEntity(entitySetName, entityId);
  ...
  private Response replaceProperty(final String location, final String accept,
      final String contentType, final String prefer, final String entitySetName,
      final String entityId, final String path, final String format, final String changes,
  ...
  private Response deletePropertyValue(final String accept, final String prefer,
      final String entitySetName, final String entityId, final String path, final String format) {
    try {
      Accept acceptType = null;
      if (StringUtils.isNotBlank(format)) {
        acceptType = Accept.valueOf(format.toUpperCase());
      ...
      final InputStream changed =
          utils.deleteProperty(entitySetName, entityId, Arrays.asList(path.split("/")), acceptType);
  ...
  public Response replacePropertyValue(@Context final UriInfo uriInfo,
      ...
      @HeaderParam("Prefer") @DefaultValue(StringUtils.EMPTY) final String prefer,
      @PathParam("entitySetName") final String entitySetName,
      @PathParam("entityId") final String entityId, @PathParam("path") final String path,
      @QueryParam("$format") @DefaultValue(StringUtils.EMPTY) final String format,
      final String changes) {

    return replaceProperty(uriInfo.getRequestUri().toASCIIString(), accept, contentType, prefer,
        entitySetName, entityId, path, format, changes, true);
  }
  ...
  public Response mergeProperty(@Context final UriInfo uriInfo,
      @HeaderParam("Accept") @DefaultValue(StringUtils.EMPTY) final String accept,
      @HeaderParam("Content-Type") @DefaultValue(StringUtils.EMPTY) final String contentType,
      @HeaderParam("Prefer") @DefaultValue(StringUtils.EMPTY) final String prefer,
      @PathParam("entitySetName") final String entitySetName,
  ...
  private Response replaceMediaProperty(final String prefer, final String entitySetName,
      final String entityId, final String path, final String value) {
      ...
      if (utils.isMediaContent(entitySetName + "/" + path)) {
        ...
        try {
          ...
          final InputStream stream =
              xml.readEntities(links.getValue(), path, links.getKey(), linkInfo.isFeed());
  ...
  private Response navigateStreamedEntity(final String entitySetName, final String entityId,
      final String path) throws Exception {

    final AbstractUtilities utils = getUtilities(null);
    final Map.Entry<String, InputStream> entityInfo =
        utils.readMediaEntity(entitySetName, entityId, path);
    return utils.createResponse(null, entityInfo.getValue(), Commons.getETag(entityInfo.getKey()),
        null);
  }

  private Response navigateProperty(final Accept acceptType, final String entitySetName,
      final String entityId, final String path, final boolean searchForValue) throws Exception {

    if ((searchForValue && acceptType != null && acceptType != Accept.TEXT)
        || acceptType == Accept.ATOM) {
    ...
    final Map.Entry<String, InputStream> entityInfo =
        utils.readEntity(entitySetName, entityId, Accept.ATOM);
    ...
    final ResWrap<Property> container = new ResWrap<Property>(
        URI.create(Constants.get(ConstantKey.ODATA_METADATA_PREFIX) + entitySetName + "(" + entityId
            + ")/" + path),
  ...
  private String stringValue(final Property property) {
    ...
    try {
      return EdmPrimitiveTypeFactory.getInstance(kind).valueToString(property.asPrimitive(), null,
          null, org.apache.olingo.commons.api.Constants.DEFAULT_PRECISION,
  ...
  public Map.Entry<Accept, AbstractUtilities> getUtilities(final String accept,
      final String format) {
    Accept acceptType;
    if (StringUtils.isNotBlank(format)) {
      try {
    ...
    return new AbstractMap.SimpleEntry<Accept, AbstractUtilities>(acceptType,
        getUtilities(acceptType));
  }
  ...
  protected void normalizeAtomEntry(final Entity entry, final String entitySetName,
      final String entityKey) {
    final org.redhelix.fit.metadata.EntitySet entitySet = metadata.getEntitySet(entitySetName);
    final EntityType entityType = metadata.getEntityOrComplexType(entitySet.getType());
    for (Map.Entry<String, org.redhelix.fit.metadata.Property> property : entityType
    ...
    for (Map.Entry<String, NavigationProperty> property : entityType.getNavigationPropertyMap()
        .entrySet()) {
      boolean found = false;
      for (Link link : entry.getNavigationLinks()) {
