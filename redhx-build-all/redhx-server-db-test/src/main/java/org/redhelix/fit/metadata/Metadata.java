...
public class Metadata extends AbstractMetadataElement {
  ...
  public Metadata(final InputStream is) {
    ...
    try {
      ...
      final XMLEventReader reader =
          ifactory.createXMLEventReader(is, org.apache.olingo.commons.api.Constants.UTF8);
      ...
      for (EntityType entityType : schemaEntry.getValue().getEntityTypes()) {
        for (NavigationProperty property : entityType.getNavigationProperties()) {
          if (StringUtils.isNotBlank(property.getReleationship())) {
            ...
            property.setType(
                property.isEntitySet() ? "Collection(" + role.getType() + ")" : role.getType());

            // let me assume that it will be just a single container
            final AssociationSet associationSet = schemaEntry.getValue().getContainers().iterator()
                .next().getAssociationSet(property.getReleationship());

            final AssociationSet.Role associationSetRole =
                associationSet.getRole(property.getToRole());
            property.setTarget(associationSetRole.getEntitySet());
          } else {
            // V4 ...
            property.setFeed(property.getType().startsWith("Collection("));

            final Collection<EntitySet> entitySets = schemaEntry.getValue().getContainers()
                .iterator().next().getEntitySets(schemaEntry.getKey(), entityType.getName());
  ...
  private Schema getSchema(final StartElement start, final XMLEventReader reader)
    ...
    while (!completed && reader.hasNext()) {
      ...
      if (event.isStartElement()
          && event.asStartElement().getName().equals(new QName(DEF_NS, "EntityType"))
        ...
        final org.redhelix.fit.metadata.Container container =
            getContainer(event.asStartElement(), reader);
        schema.addContainer(container.getName(), container);
      } else if (event.isStartElement()
          && event.asStartElement().getName().equals(new QName(DEF_NS, "Association"))) {
        // just for V3
  ...
  private org.redhelix.fit.metadata.Container getContainer(final StartElement start,
      final XMLEventReader reader) throws XMLStreamException {
    final org.redhelix.fit.metadata.Container container = new org.redhelix.fit.metadata.Container(
  ...
  private Association getAssociation(final StartElement start, final XMLEventReader reader)
      throws XMLStreamException {
    final Association association =
        new Association(start.getAttributeByName(new QName("Name")).getValue());
    ...
    while (!completed && reader.hasNext()) {
      ...
      if (event.isStartElement()
          && event.asStartElement().getName().equals(new QName(DEF_NS, "End"))) {
  ...
  private AssociationSet getAssociationSet(final StartElement start, final XMLEventReader reader)
      throws XMLStreamException {
    final AssociationSet associationSet =
        new AssociationSet(start.getAttributeByName(new QName("Name")).getValue(),
    ...
    while (!completed && reader.hasNext()) {
      ...
      if (event.isStartElement()
        ...
        final String entitySet =
            event.asStartElement().getAttributeByName(new QName("EntitySet")).getValue();
  ...
  private EntityType getEntityType(final StartElement start, final XMLEventReader reader)
    ...
    final EntityType entityType =
        new EntityType(start.getAttributeByName(new QName("Name")).getValue());
    ...
    while (!completed && reader.hasNext()) {
      ...
      if (event.isStartElement()
          && event.asStartElement().getName().equals(new QName(DEF_NS, "Property"))) {
        final org.redhelix.fit.metadata.Property property = getProperty(event.asStartElement());
  ...
  private org.redhelix.fit.metadata.Property getProperty(final StartElement start)
      throws XMLStreamException {
    final org.redhelix.fit.metadata.Property property = new org.redhelix.fit.metadata.Property(
        start.getAttributeByName(new QName("Name")).getValue());
  ...
  private EntitySet getEntitySet(final StartElement start, final XMLEventReader reader)
    ...
    while (!completed && reader.hasNext()) {
      ...
      if (event.isStartElement() && event.asStartElement().getName()
          .equals(new QName(DEF_NS, "NavigationPropertyBinding"))) {
