...
public class XMLEventReaderWrapper implements XMLEventReader {
  ...
  private static final Charset ENCODING =
      Charset.forName(org.apache.olingo.commons.api.Constants.UTF8);
  ...
  public XMLEventReaderWrapper(final InputStream stream) throws IOException, XMLStreamException {
    final StringBuilder startBuilder = new StringBuilder();
    startBuilder.append("<").append(CONTENT).append(" xmlns:m").append("=\"")
        .append(Constants.get(ConstantKey.METADATA_NS)).append("\"").append(" xmlns:d")
        .append("=\"").append(Constants.get(ConstantKey.DATASERVICES_NS)).append("\"")
        .append(" xmlns:georss").append("=\"").append(Constants.get(ConstantKey.GEORSS_NS))
        .append("\"").append(" xmlns:gml").append("=\"").append(Constants.get(ConstantKey.GML_NS))
        .append("\"").append(">");
    ...
    final InputStreamReader reader = new InputStreamReader(new ByteArrayInputStream(
        (CONTENT_STAG + IOUtils.toString(stream, ENCODING).replaceAll("^<\\?xml.*\\?>", "")
  ...
  private void init() {
    ...
    try {
      ...
      } while (nextGivenEvent.isStartDocument() || (nextGivenEvent.isStartElement()
  ...
  private boolean isValidEvent(final XMLEvent event) {
    ...
    return event != null && (!event.isEndElement()
