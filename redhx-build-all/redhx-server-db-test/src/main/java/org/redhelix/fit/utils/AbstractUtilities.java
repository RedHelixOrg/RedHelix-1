...
public abstract class AbstractUtilities {
   ...
   * @return
   */
  protected abstract InputStream addLinks(final String entitySetName, final String entitykey,
      final InputStream is, final Set<String> links) throws Exception;
   ...
   * @return
   */
  protected abstract NavigationLinks retrieveNavigationInfo(final String entitySetName,
   ...
   * @return
   */
  protected abstract InputStream normalizeLinks(final String entitySetName, final String entityKey,
      final InputStream is, final NavigationLinks links) throws Exception;

  public InputStream saveSingleEntity(final String key, final String entitySetName,
      final InputStream is, final NavigationLinks links) throws Exception {

    // -----------------------------------------
    // 0. Get the path
    // -----------------------------------------
    final String path = entitySetName + File.separatorChar + Commons.getEntityKey(key)
        + File.separatorChar + Constants.get(ConstantKey.ENTITY);
        // -----------------------------------------
    ...
    // -----------------------------------------
    final FileObject fo =
        fsManager.putInMemory(normalized, fsManager.getAbsolutePath(path, getDefaultFormat()));
    // -----------------------------------------
  ...
  public InputStream addOrReplaceEntity(final String key, final String entitySetName,
      final InputStream is, final Entity entry) throws Exception {
    ...
    for (String availableLink : new HashSet<String>(linksToBeKept)) {
      try {
        fsManager
            .resolve(Commons.getLinksPath(entitySetName, key, availableLink, Accept.JSON_FULLMETA));
      } catch (Exception e) {
    ...
    // -----------------------------------------
    // 1. Get default entry key and path (N.B. operation will consume/close the stream; use a copy
    // instead)
    // -----------------------------------------
    ...
    for (final Link link : entry.getNavigationLinks()) {
      ...
      if (navProp != null) {
        ...
        if (link.getInlineEntity() != null) {
          final String inlineEntryKey =
              getDefaultEntryKey(inlineEntitySetName, link.getInlineEntity());

          addOrReplaceEntity(inlineEntryKey, inlineEntitySetName,
              toInputStream(link.getInlineEntity()), link.getInlineEntity());
          ...
          for (Entity subentry : link.getInlineEntitySet().getEntities()) {
            ...
            addOrReplaceEntity(inlineEntryKey, inlineEntitySetName, toInputStream(subentry),
                subentry);
  ...
  public void addMediaEntityValue(final String entitySetName, final String entityKey,
      final InputStream is) throws Exception {

    // -----------------------------------------
    // 0. Get default entry key and path (N.B. operation will consume/close the stream; use a copy
    // instead)
    // -----------------------------------------
    final String path = Commons.getEntityBasePath(entitySetName, entityKey);
    // -----------------------------------------
    ...
    // -----------------------------------------
    fsManager.putInMemory(is,
        fsManager.getAbsolutePath(path + Constants.get(ConstantKey.MEDIA_CONTENT_FILENAME), null));
    IOUtils.closeQuietly(is);
    // -----------------------------------------
  }

  public void putLinksInMemory(final String basePath, final String entitySetName,
      final String entityKey, final String linkName, final Collection<String> links)
          throws Exception {
    ...
    if (navigationProperties.get(linkName) != null
  ...
  public void putLinksInMemory(final String basePath, final String entitySetName,
      final String linkName, final Collection<String> uris) throws Exception {

    fsManager.putInMemory(
        Commons.getLinksAsJSON(entitySetName,
            new SimpleEntry<String, Collection<String>>(linkName, uris)),
  ...
  public Response createResponse(final String location, final InputStream entity, final String etag,
      final Accept accept) {
  ...
  public Response createBatchResponse(final InputStream stream) {
    ...
    builder.header(Constants.get(ConstantKey.ODATA_SERVICE_VERSION),
  ...
  public Response createResponse(final String location, final InputStream entity, final String etag,
      final Accept accept, final Response.Status status) {
    ...
    builder.header("Content-Type",
        (accept == null ? "*/*" : accept.toString()) + contentTypeEncoding);
  ...
  public EntityCollection readEntitySet(final Accept accept, final InputStream entitySet)
    ...
    return (accept == Accept.ATOM || accept == Accept.XML ? atomDeserializer.toEntitySet(entitySet)
        : jsonDeserializer.toEntitySet(entitySet)).getPayload();
  }
  ...
  public ResWrap<Entity> readContainerEntity(final Accept accept, final InputStream entity)
      throws ODataDeserializerException {
    return accept == Accept.ATOM || accept == Accept.XML ? atomDeserializer.toEntity(entity)
  ...
  public Property readProperty(final Accept accept, final InputStream property)
      throws ODataDeserializerException {
    return (Accept.ATOM == accept || Accept.XML == accept ? atomDeserializer.toProperty(property)
        : jsonDeserializer.toProperty(property)).getPayload();
  }
  ...
  private String getDefaultEntryKey(final String entitySetName, final Entity entry,
    ...
    try {
      ...
      if ("OrderDetails".equals(entitySetName)) {
        ...
        if (entity.getProperty("OrderID") == null || entity.getProperty("ProductID") == null) {
          ...
          res = "OrderID=" + entity.getProperty("OrderID").asPrimitive() + ",ProductID="
              + entity.getProperty("ProductID").asPrimitive();
        }
        ...
        if (entity.getProperty("MessageId") == null || entity.getProperty("FromUsername") == null) {
          ...
          res = "FromUsername=" + entity.getProperty("FromUsername").asPrimitive() + ",MessageId="
              + entity.getProperty("MessageId").asPrimitive();
        }
        ...
        int productId;
        int productDetailId;
        if (entity.getProperty("ProductID") == null
            || entity.getProperty("ProductDetailID") == null) {
          if (Commons.SEQUENCE.containsKey(entitySetName)
              && Commons.SEQUENCE.containsKey("Products")) {
            productId = Commons.SEQUENCE.get("Products") + 1;
            productDetailId = Commons.SEQUENCE.get(entitySetName) + 1;
            res = "ProductID=" + String.valueOf(productId) + ",ProductDetailID="
                + String.valueOf(productDetailId);
          } else {
            throw new IOException(
                String.format("Unable to retrieve entity key value for %s", entitySetName));
          }
          ...
          res = "ProductID=" + entity.getProperty("ProductID").asPrimitive() + ",ProductDetailID="
              + entity.getProperty("ProductDetailID").asPrimitive();
        }
        Commons.SEQUENCE.put(entitySetName, productDetailId);
   ...
   */
  public LinkInfo readLinks(final String entitySetName, final String entityId,
      final String linkName, final Accept accept) throws Exception {
  ...
  public InputStream putMediaInMemory(final String entitySetName, final String entityId,
      final InputStream value) throws IOException {
    return putMediaInMemory(entitySetName, entityId, null, value);
  }

  public InputStream putMediaInMemory(final String entitySetName, final String entityId,
      final String name, final InputStream value) throws IOException {
    final FileObject fo =
        fsManager.putInMemory(value,
  ...
  public Map.Entry<String, InputStream> readMediaEntity(final String entitySetName,
      final String entityId) {
    return readMediaEntity(entitySetName, entityId, null);
  }

  public Map.Entry<String, InputStream> readMediaEntity(final String entitySetName,
      final String entityId, final String name) {
    final String basePath = Commons.getEntityBasePath(entitySetName, entityId);
    return new SimpleEntry<String, InputStream>(basePath, fsManager.readFile(
        basePath + (name == null ? Constants.get(ConstantKey.MEDIA_CONTENT_FILENAME) : name)));
  }

  public Map.Entry<String, InputStream> readEntity(final String entitySetName,
      final String entityId, final Accept accept) {
  ...
  public InputStream expandEntity(final String entitySetName, final String entityId,
      final String linkName) throws Exception {

    // --------------------------------
    // 0. Retrieve all 'linkName' navigation link uris (NotFoundException if missing)
    // --------------------------------
    final Map.Entry<String, List<String>> links =
        extractLinkURIs(entitySetName, entityId, linkName);
        // --------------------------------
    ...
    return readEntities(links.getValue(), linkName, links.getKey(),
        navigationProperties.get(linkName).isEntitySet());
  }

  public InputStream expandEntity(final String entitySetName, final String entityId,
      final InputStream entity, final String linkName) throws Exception {
    // --------------------------------
  ...
  public InputStream deleteProperty(final String entitySetName, final String entityId,
      final List<String> path, final Accept accept) throws Exception {
    final String basePath = Commons.getEntityBasePath(entitySetName, entityId);

    final Accept acceptType = accept == null || Accept.TEXT == accept ? Accept.XML
        : accept.getExtension().equals(Accept.JSON.getExtension()) ? Accept.JSON_FULLMETA : accept;

    // read atom
    InputStream stream =
        fsManager.readFile(basePath + Constants.get(ConstantKey.ENTITY), acceptType);
  ...
  public abstract InputStream readEntities(final List<String> links, final String linkName,
      final String next, final boolean forceFeed) throws Exception;

  protected abstract InputStream replaceLink(final InputStream toBeChanged, final String linkName,
  ...
  public abstract InputStream addEditLink(final InputStream content, final String title,
      final String href) throws Exception;

  public abstract InputStream addOperation(final InputStream content, final String name,
      final String metaAnchor, final String href) throws Exception;

  protected abstract InputStream replaceProperty(final InputStream src,
      final InputStream replacement, final List<String> path, final boolean justValue)
          throws Exception;
  ...
  public abstract Map.Entry<String, List<String>> extractLinkURIs(final String entitySetName,
      final String entityId, final String linkName) throws Exception;
}
