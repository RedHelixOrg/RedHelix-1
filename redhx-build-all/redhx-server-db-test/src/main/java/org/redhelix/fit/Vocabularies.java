...
public class Vocabularies {
  ...
  public Vocabularies() throws IOException {
    Metadata metadata = new Metadata(FSManager.instance(ODataServiceVersion.V40)
  ...
  public Response getMetadata() {
    try {
      return xml
          .createResponse(null,
              FSManager.instance(ODataServiceVersion.V40)
                  .readRes("vocabularies-" + Constants.get(ConstantKey.METADATA), Accept.XML),
              null, Accept.XML);
    } catch (Exception e) {
  ...
  public Response getVocabulary(@PathParam("vocabulary") final String vocabulary) {
    try {
      return xml.createResponse(null,
          FSManager.instance(ODataServiceVersion.V40).readFile(vocabulary, null), null, Accept.XML);
    } catch (Exception e) {
      return xml.createFaultResponse(Accept.XML.toString(), e);
    }
  }
