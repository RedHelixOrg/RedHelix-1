...
/**
 * A voltage in in a computer. This may be a value used to set power limit or it can be a reading
 * from a sensor. This is not a sensor because it does not have a sensor name or number.
 *
...
public abstract class RedHxAbstractPowerVoltageReading implements RedHxPowerVoltageReading {
  ...
  protected RedHxAbstractPowerVoltageReading(final short voltageMajor, final byte voltageMinor) {
    ...
    if (voltageMajor < MIN_VOLTAGE_MAJOR_READING) {
      throw new IllegalArgumentException("Invalid argument voltageMajor. It can not be less than "
          + MIN_VOLTAGE_MAJOR_READING + ". It was " + voltageMajor + ".");
    }

    if (voltageMajor > MAX_VOLTAGE_MAJOR_READING) {
      throw new IllegalArgumentException("Invalid argument voltageMajor. It can not be more than "
          + MAX_VOLTAGE_MAJOR_READING + ". It was " + voltageMajor + ".");
    }

    if (voltageMinor < 0) {
      throw new IllegalArgumentException(
          "Invalid argument voltageMinor. It can not be less than zero. It was " + voltageMinor
              + ".");
    }

    if (voltageMinor > MAX_VOLTAGE_MINOR_READING) {
      throw new IllegalArgumentException("Invalid argument voltageMinor. It can not be more than "
          + MAX_VOLTAGE_MINOR_READING + ". It was " + voltageMinor + ".");
    }
