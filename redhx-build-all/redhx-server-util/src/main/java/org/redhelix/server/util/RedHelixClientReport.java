...
/**
 * A simple way to test reading the Redfish Chassis JSON mockups from the DMTF. This read all Redfish chassis objects from a HTTP connection
 * and the Redfish computer systems with the chassis and prints them out on the console. To run this requires a server implementingthe
 * Redfish protocol or the Redfish mockup DMTF DSP2043_0.99.0a. In either case the the TCP port of the Redfish server must defined by system
 * properties(below). If you are using DSP2043_0.99.0a each of the Redfish mockup chassis files named index.html files has to be modified
 * and switch the field names "Name" and "Description".
 * <p>
...
public class RedHelixClientReport
{
    ...
    private static void logMissingProperty(String propertyValue,
                                           String propertyName)
    {
    ...
    /**
     * open a connection to a Redfish enabled server and read the chassis and Computer System JSON messages.
     *
     * @param protocol
     * @param hostname
     * @param portNum
                                             ...
                                             String prefix)
    {
        ...
        try
        {
            ...
            {
                /*
                 * read all the chasiss information from the Redfish server. The following three steps are the heart of this program. The form
                 * the RedHelix API and all that is necessary to convert from the Redfish JSON messages describing all the chassis and computer
                 * systems on a Redfish enabled server to Java classes. All RedHelix raaders classes have a single static method to read data. This allows them
                 ...
                 *
                 * First Step. Read the paths to each chassis in the Redfish server. There may be multiple chasisis Redfish is aware of.
                 */
                ...
                if (!chassisCollection.isEmpty())
                {
                    ...
                    /**
                     * Now all chassis and Computer System data has been read from the Redfish server so print it out on stdio. todo print
                     * on in section order. A bug is in printing in alpha order and all blades have the same prompt
                     */
                    RedHxPrintCollectionsHelper.printCollections(RedHxColumnOutputFormatter.PrintOrder.SECTION,
                                                                 chassisCollection,
            ...
            {
                Logger.getLogger(RedHelixClientReport.class.getName()).log(Level.SEVERE,
                                                                           null,
                                                                           ex);
            }
    ...
    private static void printUsage()
    {
        ...
        System.out
                .println("java -Dparam_protocol=\"http\" -Dparam_hostname=\"localhost\" -Dparam_port=\"9080\" -jar ./redhx-server-util/target/redhx-server-util-0.1-SNAPSHOT.jar");
    }
}
